...
module.exports = {
	data: new SlashCommandBuilder()
		.setName("api")
		.setDescription(
			"Returns how to use a specific BO3 GSC function with a description and example"
		)
	...
	async slash(interaction) {
		...
		const result = Object.values(parsedData).filter((user) =>
			new RegExp(command, "i").test(user.functionName)
		);
		...
		const returnValue = {
			...
			"vector[]":
				"This function returns a vector array [(x, y, z), (x, y, z), ...].",
		...
		const referenceVariable = {
			...
			"<player_or_playercorpse>":
				"Call this function on a player or player corpse.",
			...
			"<script_model/script_origin/script_brushmodel>":
				"Call this function on a script model, script origin, or script brushmodel.",
		...
		if (result.length === 0) {
			logger.debug(
				`${interaction.user.username}#${interaction.user.discriminator} searched for API function ${command} but it was not found.`
			);
			await interaction.reply({
				content: `Sorry, but I was unable to find the function \`${command}\`.`,
				ephemeral: true,
			});
		} else {
			...
			const index = functionNames.indexOf(
				closestMatch.closestMatch(command.toLowerCase(), functionNames)
			);

			const variables = result[index].variables
				.map((x) => {
					if (x.mandatory) {
						return "<" + x.name + ">";
					} else {
			...
			const apiEmbed = new Discord.EmbedBuilder()
				.setColor("#0099ff")
				.setAuthor({
					name: `<${result[index].returnValue}> ${result[index].functionName}(${variables})`,
				})
			...
			const parameters = result[index].variables
				.map((x) => {
					if (x.mandatory) {
			...
			if (result[index].example != null) {
				apiEmbed.addFields({
					name: "Example",
					value: codeBlock("c", result[index].example),
					inline: false,
				});
			}
			...
			if (result[index].section !== "") {
				apiEmbed.addFields({
					name: "Section",
					value:
						result[index].section.charAt(0).toUpperCase() +
						result[index].section.slice(1),
					inline: true,
				});
			}
			...
			if (result[index].returnValue !== "") {
				apiEmbed.addFields({
					name: "Returns",
					value: returnValue[result[index].returnValue],
					inline: true,
				});
			}
			...
			if (result[index].referenceVariable !== "") {
				apiEmbed.addFields({
					...
					value: referenceVariable[result[index].referenceVariable],
					inline: true,
				});
			}
			...
			if (result.length > 1) {
				...
				apiEmbed.addFields({
					name:
						...
						"` here are they:",
					value:
						functionNames.filter((item) => item).join(", ") +
						" " +
						(result.length < 8
							? ""
							: " ... and " + (closeMatches - 8) + " more."),
					inline: false,
				});
			}
