...
module.exports = class Update {
	...
	async execute(message, client, bot) {
		...
		if (!updateFile) {
			await message.reply({ content: 'You\'ve not included the target you\'d like to use to update', allowedMentions: { repliedUser: false } });
		...
		if (!files.includes(updateFile + '.json')) {return await message.reply({ content: `${updateFile} does not exist. Use \`+update\` to find available targets`, allowedMentions: { repliedUser: false } });}
