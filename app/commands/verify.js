	...
	execute(message, client, bot) {
		...
		if (!message.content.split(" ")[1]) {
			return message.channel.send(
				`You didn't provide a user, ${message.author}!`
			);
		}

		if (!member) {
			return message.channel.send(
				`Unable to get information about that user, ${message.author}!`
			);
		}
			...
			async (err, res, data) => {
				...
				if (_.isEmpty(data)) {
					await message.channel.send(
						`${member} is not verified, the user might be verified on another Discord account.`
					);
					return;
				}
				...
				if (data.hasOwnProperty("error")) {
					await message.reply({
						content: `Sorry, but I'm not able to check the status of ${member} at the moment.`,
						allowedMentions: { repliedUser: false },
					});
					return;
				} else {
					await message.channel.send(
						`${member} got verified at \`${data[0].created_at}\`.`
					);
